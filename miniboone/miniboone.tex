...
In our final event selection, we require that the sum of the energy deposited be more than $4.5$ MeV$/ 1.7$ cm, compatible with an $e^+e^-$ pair and yielding an efficiency of $90\%$. We obtain the expected size of neutrino-electron scattering and background events in this range from Fig. 3 of~\cite{Park:2015eqa}. These datasets are shown in \reffig{fig:NP_events} for CHARM-II and \minerva ME.
...
In~\reffig{fig:final_plot} we show the new limits we derive on coherent forward gammas as a bound in the $|U_{\mu 4}|$ vs $m_4$ plane of Ref.~\cite{Bertuzzo:2018itn} at 90\% confidence level, together with the MiniBooNE fit from~\cite{Bertuzzo:2018itn}.

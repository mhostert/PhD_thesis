...
\hline
		Channel & T2K-I& T2K-II & MINOS & MINOS+ & NO$\nu$A-I & NO$\nu$A-II & MINER$\nu$A \\ \hline \hline
...
\end{tabular}}
\end{center}
\caption[Trident rates at non-LAr detectors.]{\label{tab:otherrates}Total number of \textbf{coherent} (top row) and \textbf{incoherent} (bottom row) trident events expected at different non-LAr detectors for each channel. The numbers in parentheses are for the antineutrino running mode, when present. These calculations consider a detection efficiency of 100\%.}
\end{table}